...
import StyledCtsCtsTestIconButtonUrlWrapper from './styles';

// interface for props
interface CtsCtsTestIconButtonUrlProps extends PConnFieldProps {
...
function CtsCtsTestIconButtonUrl(props: CtsCtsTestIconButtonUrlProps) {
  ...
  }, [context, countryCode, showTable]);

  // when DISPLAY_ONLY, in this example we will just show the URL.
  if (displayMode === 'DISPLAY_ONLY') {
    const displayComp = textFormatter(validUrl);
    return (
      <StyledCtsCtsTestIconButtonUrlWrapper> {displayComp} </StyledCtsCtsTestIconButtonUrlWrapper>
    );
  }

  return (
    <StyledCtsCtsTestIconButtonUrlWrapper>
    <div style={{ display: 'flex' }} data-testid={testId}>
      ...
      {showTable && (
        <Table
          ...
          data={statelist}
          loading={isLoading}
          loadingMessage={PConnect.getLocalizedValue('Loading state list', '', '')}
        />
      )}
    </StyledCtsCtsTestIconButtonUrlWrapper>
  );
...
export default withConfiguration(CtsCtsTestIconButtonUrl);
